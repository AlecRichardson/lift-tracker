...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function renderWorkout(loadLast = false) {
    ...
    let lastSaved = null;
    if (loadLast) {
      const dayLogs = logs.filter(l => l.d === currentDay);
      if (dayLogs.length) lastSaved = dayLogs[dayLogs.length - 1].e;
    }

    const draft = getDraft();
    const template = window.templates[currentDay] || [];

    template.forEach((name, exIndex) => {
      const exDiv = document.createElement("div");
      exDiv.className = "exercise";
      exDiv.innerHTML = `<h4>${name}</h4>`;

      for (let i = 0; i < 3; i++) {
        const row = document.createElement("div");
        row.className = "setRow";
        const reps = document.createElement("input");
        reps.type = "number"; reps.placeholder = "Reps";
        const weight = document.createElement("input");
        weight.type = "number"; weight.placeholder = "Weight";

        // Prefill last workout or draft
        if (lastSaved?.[exIndex]?.s?.[i]) {
          reps.value = lastSaved[exIndex].s[i].reps || "";
          weight.value = lastSaved[exIndex].s[i].weight || "";
        } else if (draft?.[exIndex]?.s?.[i]) {
          reps.value = draft[exIndex].s[i].reps || "";
          weight.value = draft[exIndex].s[i].weight || "";
        }
  ...
  document.addEventListener("input", () => {
    ...
    document.querySelectorAll(".exercise").forEach(ex => {
      ...
      draft.push({ n: ex.querySelector("h4").textContent, s: sets });
    });
    saveDraft(draft);
  });
  ...
  async function renderChart() {
    ...
    logs.forEach(log => {
      log.e.forEach(ex => {
        ex.s.forEach(set => {
          ...
          dataPoints.push({ x: new Date(log.t), y: Math.round(est1RM) });
        });
      });
    });
    ...
    chart = new Chart(ctx, {
      type: "line",
      data: { datasets: [{ label: "Estimated 1RM", data: dataPoints, borderColor: "#7289da", tension: 0.3 }] },
      options: { responsive: true, plugins: { legend: { display: true } }, scales: { y: { ticks: { color: "#fff" } }, x: { ticks: { color: "#fff" } } } }
    });
  }
  ...
  function formatTimestamp(ts) {
    ...
    return date.toLocaleString("en-US", {
      month: "long", day: "numeric", weekday: "long", hour: "numeric", minute: "2-digit"
    });
  }
